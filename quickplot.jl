...
function main()
  energies = CSV.read("./data/energies.csv", DataFrame, header=1)
  correlations = CSV.read("./data/correlations.csv", DataFrame, header=true)

  @df energies plot( :Energy, leg=false, title="n = "*string(nrow(correlations)), xlabel="epoch", ylabel="⟨H⟩", 
                     dpi=300 )
  savefig("./data/energies.png")
  ...
  if ( mod(nrow(correlations), 2) == 1 )
    ...
    gif(anim, "./data/correlations.gif", fps = 20)
