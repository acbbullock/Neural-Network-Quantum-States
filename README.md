...
as a constant average of the logarithmic derivatives $O_l(s,\alpha)$ over the drawn samples $\tilde{S}$, we may decouple the $O_l(s',\alpha)$ from the inner sum of $E_{\text{loc}}(s, \alpha)$ over all $s' \in S$ such that
    $$F_{\alpha_l} \approx \frac{1}{N} \sum_{s \in \tilde{S}} O_l(s,\alpha) E_{\text{loc}}(s, \alpha) - \bigg[ \frac{1}{N} \sum_{s \in \tilde{S}} O_l(s,\alpha) \bigg] \bigg[ \frac{1}{N} \sum_{s \in \tilde{S}} E_{\text{loc}}(s, \alpha) \bigg]$$
is the covariance of the logarithmic derivatives $O_l(s,\alpha)$ and the local energies $E_{\text{loc}}(s, \alpha)$. In operator notation, we may express the forces as the expectation covariances
    $$F_{\alpha_l} = \langle O_l H \rangle_{\psi(\alpha)} - \langle O_l \rangle_{\psi(\alpha)} \langle H \rangle_{\psi(\alpha)} = \frac{\langle \psi(\alpha), O_lH\psi(\alpha) \rangle}{\langle \psi(\alpha), \psi(\alpha) \rangle} - \frac{\langle \psi(\alpha), O_l\psi(\alpha) \rangle}{\langle \psi(\alpha), \psi(\alpha) \rangle} \frac{\langle \psi(\alpha), H\psi(\alpha) \rangle}{\langle \psi(\alpha), \psi(\alpha) \rangle} = \frac{\sum_{s \in S} \psi^\*(s,\alpha) O_l(s,\alpha) E_{\text{loc}}(s, \alpha)}{\sum_{s' \in S} |\psi(s',\alpha)|^2} - \bigg[ \frac{\sum_{s \in S} \psi^\*(s,\alpha) O_l(s,\alpha)}{\sum_{s' \in S} |\psi(s',\alpha)|^2} \bigg] \bigg[ \sum_{s \in S} p(s,\alpha) E_{\text{loc}}(s,\alpha) \bigg] \approx \frac{1}{N} \sum_{s \in \tilde{S}} O_l(s,\alpha) E_{\text{loc}}(s, \alpha) - \bigg[ \frac{1}{N} \sum_{s \in \tilde{S}} O_l(s,\alpha) \bigg] \bigg[ \frac{1}{N} \sum_{s \in \tilde{S}} E_{\text{loc}}(s, \alpha) \bigg]$$
of the operators $O_l$ and $H$. The parameters $\alpha$ are updated according to the stochastic optimization update rule
